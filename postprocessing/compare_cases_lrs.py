...
#_caselist = [os.path.join(reeds_path,'postprocessing','example_high_low.csv')]
#_caselist = [os.path.join(reeds_path,'postprocessing','example_ftl_stscen_highRE_reduced.csv')]
#_caselist = [os.path.join(reeds_path,'postprocessing','example_ftl_stscen_LowRE_reduced.csv')]
#_caselist = ['/projects/txraneeds/lserpe/analysis/example_scenarios/example_ftl_lowerRE_report.csv']
#_caselist = ['C:\\Users\\LSERPE\\Documents\\Repo\\hackathon\\ReEDS-2.0\\postprocessing\\example_ftl_core.csv']
# _caselist = ['C:\\Users\\LSERPE\\Documents\\Repo\\hackathon\\ReEDS-2.0\\postprocessing\\example_txneeds_lowRE_all_FTL.csv']
#_caselist = ['C:\\Users\\LSERPE\\Documents\\Repo\\hackathon\\ReEDS-2.0\\postprocessing\\example_txneeds_lowRE_all_FTL_grouped.csv']
#_caselist = ['C:\\Users\\LSERPE\\Documents\\Repo\\hackathon\\ReEDS-2.0\\postprocessing\\example_txneeds_lowRE_zero_FTL.csv']
#_caselist = ['C:\\Users\\LSERPE\\Documents\\Repo\\hackathon\\ReEDS-2.0\\postprocessing\\example_txneeds_lowRE_zero_constant_FTL.csv']
_caselist = ['C:\\Users\\LSERPE\\Documents\\Repo\\hackathon\\ReEDS-2.0\\postprocessing\\example_txneeds_lowRE_zero_high_FTL.csv']
#_caselist = ['C:\\Users\\LSERPE\\Documents\\Repo\\hackathon\\ReEDS-2.0\\postprocessing\\example_txneeds_lowRE_sensitivities_const_FTL.csv']
#_caselist = ['C:\\Users\\LSERPE\\Documents\\Repo\\hackathon\\ReEDS-2.0\\postprocessing\\example_txneeds_lowRE_sensitivities_high_FTL.csv']
#_caselist = ['/projects/txraneeds/lserpe/analysis/example_scenarios/example_txneeds_lowRE_sensitivities_const_highFTL.csv']
# # _caselist = ["Z:\\FY25_LRS_Tx_needs\\runs_for_feb\\example_txneeds_highRE_core.csv"]
#_caselist = ["Z:\\FY25_LRS_Tx_needs\\runs_for_feb\\example_txneeds_high vs low RE.csv"]
#_caselist = ["Z:\\FY25_LRS_Tx_needs\\runs_for_feb\\example_txneeds_highRE_sensitivities_high_load.csv"]
#_caselist = ["Z:\\FY25_LRS_Tx_needs\\runs_for_feb\\example_txneeds_highRE_sensitivities_noEPA111.csv"]
#_caselist = ["Z:\\FY25_LRS_Tx_needs\\runs_for_feb\\example_txneeds_highRE_sensitivities_noHR.csv"]
#_caselist = ["Z:\\FY25_LRS_Tx_needs\\runs_for_feb\\example_txneeds_highRE_sensitivities_decarb.csv"]
#_caselist = ['/projects/txraneeds/lserpe/repo3/ReEDS-2.0/postprocessing/example_txneeds_lowRE_sensitivities_high_load.csv']
#_caselist = ["/projects/txraneeds/lserpe/repo3/ReEDS-2.0/postprocessing/example_txneeds_lowRE_sensitivities_noEPA111.csv"]
#_caselist = ['/projects/txraneeds/lserpe/repo3/ReEDS-2.0/postprocessing/example_txneeds_lowRE_sensitivities_noHR.csv']
#_caselist = ["/projects/txraneeds/lserpe/repo3/ReEDS-2.0/postprocessing/example_txneeds_lowRE_sensitivities_decarb.csv"]
#_caselist = ["/projects/txraneeds/lserpe/repo3/ReEDS-2.0/postprocessing/example_txneeds_lowRE_sensitivities_vsc.csv"]
#_caselist = ["/projects/txraneeds/lserpe/repo3/ReEDS-2.0/postprocessing/example_txneeds_lowRE_sensitivities_zero_highFTL.csv"]
#_caselist = ["/projects/txraneeds/lserpe/repo3/ReEDS-2.0/postprocessing/example_txneeds_lowRE_sensitivities_const_highFTL.csv"]
#_caselist = ["/projects/txraneeds/lserpe/analysis/example_scenarios/example_txneeds_lowRE_sensitivities_zero_highFTL.csv"]
_casenames = ''
titleshorten = 0
startyear = 2020
sharey = 'row'
...
def plot_bars_abs_stacked(
    ...
    * dfplot must have cases as rows and stacked bar elements (matching colors) as cols
    ...
    for (row, df) in enumerate([dfplot, dfdiff]):
        plots.stackbar(df=df, ax=ax[row], colors=colors, net=(net or row), width=0.8)
        ymin, ymax = ax[row].get_ylim()
...
def plot_bars_abs_stacked_only_diff(
        dfplot, basecase, colors, ax, col=0,
        net=True, label=True, ypad=0.02, fontsize=9,
    ):
    """
    ...
    * dfplot must have cases as rows and stacked bar elements (matching colors) as cols
    ...
    dfdiff = dfdiff.loc[~(dfdiff == 0).all(axis=1)]
    for (row, df) in enumerate([dfdiff]):
        plots.stackbar(df=df, ax=ax, colors=colors, net=(net or row), width=0.8)
...
#hierarchy = reedsplots.get_hierarchy('/projects/txraneeds/lserpe/repo3/ReEDS-2.0/runs/v1_021425_USA_BAU_LowRE_zeroFTL')
hierarchy = reedsplots.get_hierarchy("Z:\\FY25_LRS_Tx_needs\\runs for march\\v1_021425_USA_BAU_LowRE_zeroFTL\\")
#hierarchy = pd.read_csv('/projects/txraneeds/lserpe/analysis/hierarchy.csv')



# for transgrp_nercr plots use this dfmap to avoid changing the hierarchy file in runs that already finished 
#dfmap =reedsplots.get_dfmap('/projects/txraneeds/lserpe/repo3/ReEDS-2.0/runs/v1_021425_USA_BAU_LowRE_zeroFTL')
dfmap = reedsplots.get_dfmap('Z:\\FY25_LRS_Tx_needs\\runs for march\\v1_021425_USA_BAU_LowRE_zeroFTL\\')
...
for case in tqdm(cases, desc='regional transmission'):
    ...
    for _level in ['interconnect','transreg','transgrp','st','transgrp_nercr']:
...
aggstack = {
    ...
    **{
        ...
        'gas-cc-ccs_mod':'Gas + Gas-CCS',
        'coal-ccs_mod':'Coal + Coal-CCS',
        'gas-cc':'Gas + Gas-CCS', 'gas-ct':'Gas-CT', 'o-g-s':'Gas + Gas-CCS',
        'coal':'Coal + Coal-CCS',
        ...
        'csp':'Solar',
        'upv':'Solar', 'dupv':'Solar', 'distpv':'Solar',
        'pvb':'Solar',
...
aggcolors = {
    ...
    #'Gas':plt.cm.tab20(8),
    'Gas + Gas-CCS':techcolors['gas-cc'],
    'Gas-CT':techcolors['gas-ct'],
    'Coal + Coal-CCS':'C7',
    'Gas+CCS':techcolors['gas-cc-ccs_mod'],
...
plt.close()
f,ax = plt.subplots(
    1, 1, figsize=(8, 5), sharex=True,
    sharey=('col' if (sharey is True) else False),
)
handles = {}
### Final capacity difference

# basemap2 = {'Core zero FTL': 'Core zero FTL',
#             'Core high FTL': 'Core zero FTL',
#             'Core zero FTL high demand': 'Core zero FTL high demand',
#             'Core high FTL high demand': 'Core zero FTL',
#             'Higher RE zero FTL': 'Higher RE zero FTL',
#             'Higher RE high FTL': 'Core zero FTL',
#             'Core zero FTL CO2 constrained': 'Core zero FTL CO2 constrained',
#             'Core high FTL CO2 constrained': 'Core zero FTL',
#             'Core zero FTL HVDC': 'Core zero FTL HVDC',
#             'Core high FTL HVDC': 'Core zero FTL'}

# basemap_filtered = {'Zero FTL': 'Zero FTL',
#                     'Constant FTL': 'Zero FTL',
#                     'Zero FTL high demand': 'Zero FTL high demand',
#                     'Constant FTL high demand': 'Zero FTL high demand',
#                     'Zero FTL Higher RE': 'Zero FTL Higher RE',
#                     'Constant FTL Higher RE': 'Zero FTL Higher RE',
#                     'Zero FTL CO2 constrained': 'Zero FTL CO2 constrained',
#                     'Constant FTL CO2 constrained': 'Zero FTL CO2 constrained',
#                     'Zero FTL HVDC': 'Zero FTL HVDC',
#                     'Constant FTL HVDC': 'Zero FTL HVDC'}
basemap_filtered = {'Zero FTL': 'Zero FTL',
                    'High FTL': 'Zero FTL',
                    'Zero FTL high demand': 'Zero FTL high demand',
                    'High FTL high demand': 'Zero FTL high demand',
                    'Zero FTL Higher RE': 'Zero FTL Higher RE',
                    'High FTL Higher RE': 'Zero FTL Higher RE',
                    'Zero FTL CO2 constrained': 'Zero FTL CO2 constrained',
                    'High FTL CO2 constrained': 'Zero FTL CO2 constrained',
                    'Zero FTL HVDC': 'Zero FTL HVDC',
                    'High FTL HVDC': 'Zero FTL HVDC'}
...
dictin_cap_filtered = {k: v for k, v in dictin_cap.items() if 'Zero FTL' in k}
toplot = {
    'Capacity': {
        'data': dictin_cap,
        'values':'Capacity (GW)',
...
#ax[0].axhline(0, c='k', ls='--', lw=0.75)
#ax.axhline(0, c='k', ls='--', lw=0.75)
for col, (datum, data) in enumerate(toplot.items()):
    #ax[col].set_ylabel(data['label'], y=-0.075)
    ax.set_ylabel(data['label'], y=-0.075)
    dfplot = pd.concat(
        {case:
         data['data'][case].loc[data['data'][case].year==lastyear]
         .set_index('tech')[data['values']]
    ...
    #     dfplot=dfplot, basecase=basemap_filtered,
    #     colors=aggcolors, fontsize=8,
    #     ax=ax, col=col, net=True,
    #     label=(False if lesslabels else True),
    # )
    handles[datum] = plot_bars_abs_stacked_only_diff(
        dfplot=dfplot, basecase=basemap_filtered,
        colors=aggcolors, fontsize=8,
        ax=ax, col=col, net=True,
        label=(False if lesslabels else True),
...
# ax[1].set_xticks(range(len(cases)))
# ax[1].set_xticklabels(cases.keys(), rotation=90)
# ax[1].annotate('Diff', (0.03,0.03), xycoords='axes fraction', fontsize='large')
# ax[1].axhline(0, c='k', ls='--', lw=0.75)
ax.set_xticks(range(int(len(cases)/2)))
#ax.set_xticklabels([k for k in cases.keys() if 'Constant FTL' in k], rotation=45)
ax.set_xticklabels(['Core','High Demand','Higher RE','C02 Constrained','HVDC'], rotation=45)
ax.annotate('Diff', (0.03,0.03), xycoords='axes fraction', fontsize='large')
ax.axhline(0, c='k', ls='--', lw=0.75)
plt.ylabel('Diff in 2040 Capacity [GW]')
plt.tight_layout()
plots.despine(ax)
plt.draw()
...
plt.close()
f,ax = plt.subplots(
    2, 4, figsize=(8, 6.88), sharex=True,
    sharey=('col' if (sharey is True) else False),
)
handles = {}
...
for col, pollutant in enumerate(['CO2']):
    note = []
    df = {}
    for case in cases:
...
ylabels = {
    'transgrp_nercr': 'Transmission capacity between\nplanning regions [GW]',
...
#for _col, level in enumerate(['transgrp','interconnect']):
for _col, level in enumerate(['transgrp_nercr']):
    col = _col + 2
    df = {}
    for case in cases:
...
#%%
# Interregional stacked bars

# all in one with grouped cases by FTL
...
df_interregional = pd.DataFrame(df).T
df_interregional = df_interregional.drop(columns = [2020,2025,2030,2035])
df_interregional['category'] = df_interregional.index.map(
                        lambda x: 'Zero FTL' if 'Zero' in str(x) else 
                         ('Constant FTL' if 'Constant' in str(x) else 'High FTL')
                        )
color_map = {'Zero FTL': 'red', 'Constant FTL': 'green', 'High FTL': 'blue'}
colors = df_interregional['category'].map(color_map)

plt.figure(figsize=(8, 6))
plt.bar(df_interregional.index, df_interregional[2040], color=colors)
plt.xticks(rotation=90)
legend_labels = [mpatches.Patch(color=color_map['Zero FTL'], label='Zero FTL'),
                 mpatches.Patch(color=color_map['Constant FTL'], label='Constant FTL'),
                 mpatches.Patch(color=color_map['High FTL'], label='High FTL')]

plt.legend(handles=legend_labels)
plt.axhline(y=150, color='black', linestyle='--', linewidth=2, label="Existing Capacity")
plt.text(x=17.5, y=150, s='Existing Capacity', ha='center', color='black', fontsize=14)
plt.ylabel('Interregional Transmission \nCapacity [GW]')



# separate exisitng and new 
df_interregional = pd.DataFrame(df).T
df_interregional['existing'] = df_interregional[2020]
df_interregional['new'] = df_interregional[2040] - df_interregional[2020]
df_interregional = df_interregional.drop(columns = [2020,2025,2030,2035,2040])

df_interregional.plot.bar(stacked = True )
plt.legend().set_title("")
plt.ylabel('Interregional Transmission \nCapacity [GW]')
## uncomment for comparison against a refernce case
# df_interregional_diff = df_interregional
# df_interregional_diff['existing diff'] = df_interregional['existing'] - df_interregional['existing'][0]
# df_interregional_diff['new diff'] = df_interregional['new'] - df_interregional['new'][0]
# df_interregional_diff = df_interregional_diff.drop(columns = ['existing', 'new'])

# uncomment for comparison of sensitivity cases against each other 
df_interregional_diff = df_interregional
df_interregional_diff['existing diff'] = 0
df_interregional_diff['new diff'] = 0

for idx,row in df_interregional_diff.iterrows():
    if 'zero FTL' in idx : 
        df_interregional_diff.loc[idx,'existing diff'] = df_interregional.loc[idx]['existing'] -df_interregional.loc[idx]['existing'] 
    else:
        df_interregional_diff.loc[idx,'existing diff'] = df_interregional.loc[idx]['existing'] -df_interregional.loc[idx.replace('high FTL','zero FTL')]['existing'] 
    
    if 'zero FTL' in idx: 
        df_interregional_diff.loc[idx,'new diff'] = df_interregional.loc[idx]['new'] -df_interregional.loc[idx]['new'] 
    else:
        df_interregional_diff.loc[idx,'new diff'] = df_interregional.loc[idx]['new'] -df_interregional.loc[idx.replace('high FTL','zero FTL')]['new'] 
...
df_interregional_diff.plot.bar(stacked = True, legend = False )
plt.ylabel('Difference in Interregional \nTransmission Capacity [GW]')
# ax[1].bar(df_interregional_diff,stacked = True )
...
try:
    f, ax, dfplot = reedsplots.plot_interreg_transfer_cap_ratio(
        ...
        level='transgrp', tstart=startyear,
        ymax=None,
    )
    ### Save it
    slide = add_to_pptx(
...
try:
    f, ax, dfplot = reedsplots.plot_max_imports(
        ...
        colors={v: colors[k] for k,v in cases.items()},
        casenames={v:k for k,v in cases.items()},
        level='transgrp_nercr2', tstart=startyear,draw_limit=True, units='%',hierarchy_alt = hierarchy,dfmap=dfmap,
    )
    vmax_map = dfplot.max()
    ### Save it
    slide = add_to_pptx(
    ...
    # Plot 2040 bar
    f, ax, dfplot = reedsplots.plot_max_imports_bar(
        case=list(cases.values()),
        colors={v: colors[k] for k,v in cases.items()},
        casenames={v:k for k,v in cases.items()},
        level='transgrp_nercr2', tstart=startyear,draw_limit=True, 
        bar = True, hierarchy_alt = hierarchy, dfmap = dfmap, units = '%',
    )
    vmax_map = dfplot.max()
    # Plot 2020 and 2040 as line
    f, ax, dfplot = reedsplots.plot_max_imports_bar(
        case=list(cases.values()),
        colors={v: colors[k] for k,v in cases.items()},
        casenames={v:k for k,v in cases.items()},
        level='nercr', tstart=startyear,draw_limit=True, bar = False, units = 'MW',
    )

    # Plot annual imports
    f, ax, dfplot = reedsplots.plot_annual_imports(
    case=list(cases.values()),
    colors={v: colors[k] for k,v in cases.items()},
    casenames={v:k for k,v in cases.items()},
    level='transgrp_nercr', tstart=startyear,draw_limit=False, units = 'GW'
    )

#%%
# regional net imports maps

# annual net imports in 2040
for case in cases:

    # reedsplots.map_net_imports(case=cases[case], level='transgrp_nercr2', period = 'stress',
    #                            yearmin=2020, title =case, hierarchy_alt= hierarchy, dfmap = dfmap)
    #map_net_imports(case=cases[case], level='transgrp_nercr2', period = 'stress',yearmin=2040, title =case, hierarchy_alt= hierarchy )
    # reedsplots.map_net_imports(case=cases[case], level='transgrp_nercr2', period = None,
    #                            yearmin=2020, title =case, hierarchy_alt= hierarchy, dfmap = dfmap)
    map_net_imports(case=cases[case], level='transgrp_nercr2', period = None,
                               yearmin=2040, title =case, hierarchy_alt= hierarchy, dfmap = dfmap)
    plt.show()
    
# max net imports 
for case in cases:
    # reedsplots.map_max_net_imports(
    #         case=cases[case], level='transgrp_nercr2',
    #         cmap=plt.cm.RdBu_r, yearmin=2040, 
    #         vlim='shared', trans=False, period = 'stress',title = case,hierarchy_alt = hierarchy,dfmap = dfmap, units = 'GW',vmax = vmax_map,
    #     )
    map_max_net_imports(
            case=cases[case], level='transgrp_nercr2',
            cmap=plt.cm.Reds, yearmin=2040, 
            vlim='shared', trans=False, period = 'stress',title = case,hierarchy_alt = hierarchy,dfmap = dfmap, units = '%',vmax = vmax_map,
        )
...
#     #     pcalabel=False,
#     #     wscale=0.0004,
#     #     drawzones=False, drawstates=False,
...
#         year=lastyear,
#         alpha=1, dpi=150,
#         titleshorten=titleshorten,thickborders='transgrp' ,
#         group_trtypes=True, remove_intraiso = True, hierarchy_alt = hierarchy,dfmap = dfmap,
#     )
...
# else:
    ### Absolute
#%%
# get min, median, max tx builds across scenarios 
wscale = 0.0003
alpha = 0.8
tx_data = {}
for subtract_baseyear in [2020]:
    plt.close()
    f,ax = plt.subplots(
    ...
    for case in cases:
        ...
        fig,axes, tx_data[case] = reedsplots.plot_trans_onecase(
            ...
            thickborders='transgrp_nercr', drawstates=False, drawzones=False, 
            label_line_capacity=1,
            scale=(True if case == basecase else False),
            level = 'transgrp_nercr',group_trtypes=True, remove_intraiso = True,
            hierarchy_alt = hierarchy,dfmap = dfmap,
        )
        ax[coords[case]].set_title(case)
    ### Formatting
    title = (
        f'New interzonal transmission since {subtract_baseyear}' if (subtract_baseyear and subtract_baseyear !='existing')
        else f'Existing transmission' if subtract_baseyear =='existing'
...
#tx_data = pd.DataFrame.from_dict(tx_data, orient = 'index').reset_index()
tx_df =  pd.concat(tx_data.values(), keys = tx_data.keys())

unique_interfaces = (tx_df['r']+'-'+tx_df['rr']).unique()
tx_df['interfaces'] = (tx_df['r']+'-'+tx_df['rr'])
interface_rx_map = tx_df.set_index('interfaces')['r_x'].to_dict()
interface_ry_map = tx_df.set_index('interfaces')['r_y'].to_dict()
interface_rrx_map = tx_df.set_index('interfaces')['rr_x'].to_dict()
interface_rry_map = tx_df.set_index('interfaces')['rr_y'].to_dict()

df = pd.DataFrame()
df['r'] = unique_interfaces
df['r_x'] = df['r'].map(interface_rx_map)
df['r_y'] = df['r'].map(interface_ry_map)
df['rr_x'] = df['r'].map(interface_rrx_map)
df['rr_y'] = df['r'].map(interface_rry_map)
df[['r','rr']] = df['r'].str.split('-',expand = True)
df = df[['r','rr', 'r_x', 'r_y', 'rr_x', 'rr_y']]

merged_df = df
cases_columns = []
for case in cases:
    merged_df = pd.merge(merged_df,tx_data[case], on=['r','rr', 'r_x', 'r_y', 'rr_x', 'rr_y'], how ='outer').fillna(0)
    merged_df = merged_df.rename(columns={'MW': f'{case}_MW'})
    cases_columns.append(f'{case}_MW')

# add columns for min, mean, max 
merged_df['min'] = merged_df[cases_columns].min(axis=1)
merged_df['median'] = merged_df[cases_columns].median(axis=1)
merged_df['max'] = merged_df[cases_columns].max(axis=1)

#min
dfin = merged_df[['r','rr','min', 'r_x', 'r_y', 'rr_x', 'rr_y']]
dfin = dfin.rename(columns={'min':'MW'})

#median
dfin = merged_df[['r','rr','median', 'r_x', 'r_y', 'rr_x', 'rr_y']]
dfin = dfin.rename(columns={'max':'MW'})

#mean
dfin = merged_df[['r','rr','max', 'r_x', 'r_y', 'rr_x', 'rr_y']]
dfin = dfin.rename(columns={'max':'MW'})

f,ax = plt.subplots(
    1, 1, figsize=(13.33, 6.88),
    gridspec_kw={'wspace':0.0,'hspace':-0.1},
)

f,ax,dfplot = reedsplots.plot_trans_onecase(
            case=None, dfin = dfin, pcalabel=False, wscale=wscale,
            yearlabel=False, year=lastyear, simpletypes=None,
            alpha=alpha, scalesize=8,
            ...
            scale=(True if case == basecase else False),
            level = 'transgrp_nercr',group_trtypes=True, remove_intraiso = True,
            hierarchy_alt = hierarchy,dfmap = dfmap, user_title = 'Min',
        )

plt.close()
f,ax = plt.subplots(
   1, 3, figsize=(13.33, 6.88),
    gridspec_kw={'wspace':0.0,'hspace':-0.1},
)
idx = 0
for stat in ['min','median','max']:
  
        dfin = merged_df[['r','rr',stat, 'r_x', 'r_y', 'rr_x', 'rr_y']]
        dfin = dfin.rename(columns={stat:'MW'})
        user_title = stat.capitalize()
        ### Plot it
        plot_trans_onecase(
                case=None, dfin = dfin,pcalabel=False, wscale=wscale,
                yearlabel=False, year=lastyear, simpletypes=None,
                alpha=alpha, scalesize=8,
                f=f, ax=ax[idx], title=False,
                subtract_baseyear=subtract_baseyear,
                thickborders='transgrp_nercr', drawstates=False, drawzones=False, 
                label_line_capacity=1,
                scale=True,
                level = 'transgrp_nercr',group_trtypes=True, remove_intraiso = True,
                hierarchy_alt = hierarchy,dfmap = dfmap,user_title =user_title ,
            )
        idx+=1
...
wscale = 0.0003
alpha = 0.8
for subtract_baseyear in [None, 2020, 'existing']:
    plt.close()
    f,ax = plt.subplots(
        nrows, ncols, figsize=(13.33, 6.88),
        gridspec_kw={'wspace':0.0,'hspace':-0.1},
...
for case in cases:
    ...
    if case == basecase:
        ...
        #plot_trans_onecase(
            ...
            thickborders='transgrp_nercr', drawstates=False, drawzones=False, 
            label_line_capacity=1,
            scale=(True if case == basecase else False),level = 'transgrp_nercr',
            group_trtypes=True, remove_intraiso = True,hierarchy_alt = hierarchy,dfmap = dfmap,
        )
    else:
        ### Plot the difference
        ...
        #plot_trans_diff(
            ...
            thickborders='transgrp_nercr', drawstates=False, drawzones=False, 
            label_line_capacity=1,
            scale=False,level = 'transgrp_nercr', justdiff=True,
            group_trtypes=True, remove_intraiso = True,hierarchy_alt = hierarchy,dfmap = dfmap,
        )
### Formatting
...
#             cmap=cmocean.cm.rain,level = 'transgrp',
#         )
#         ax[0].annotate(
#             casebase_name,
...
#             cmap=plt.cm.RdBu_r,level = 'transgrp',
#         )
...
for tech in maptechs:
    ...
    for case in cases:
        ...
        if level not in ['r','rb']:
            dfba.geometry = dfba.buffer(0.)
            dfba = dfba.dissolve(level)
            dfba['x'] = dfba.centroid.x
            dfba['y'] = dfba.centroid.y
            dfba =dfba.reset_index()
            dfba = dfba.rename(columns = {level:'r'})
            dfba['r'] = dfba['r'].astype(str)
            hierarchy = pd.read_csv(
                os.path.join(casebase,'inputs_case','hierarchy.csv')
                ).rename(columns={'*r':'r'}).set_index('r')

            dfplot = dfplot.reset_index()
            dfplot['r'] =dfplot['aggreg'].map(hierarchy[level])                
            dfplot = dfplot.groupby(['r'], as_index=False).GW.sum()

            dfplot = dfba.merge(dfplot, on='r').fillna(0).reset_index(drop=True)
            dfthick = dfmap[level]
            dfthick.geometry = dfthick.buffer(0.)
            dfthick = dfthick.dissolve(level)
            dfthick.plot(ax=ax[coords[case]], edgecolor='k', facecolor='none', lw=0.25)
            ax[coords[case]].axis('off')
            dfplot.plot(
        ...
        if coords[case] == legendcoords:
            plots.addcolorbarhist(
                ...
                orientation='horizontal', labelpad=2.0, histratio=0.,
...
for tech in maptechs:
    ...
    plots.addcolorbarhist(
        ...
        title=f'{tech} {lastyear}capacity [GW]',
