...
#_caselist = ["Z:\\FY25_LRS_Tx_needs\\runs_for_feb\\example_txneeds_higher RE FTL variation.csv"]
...
#_caselist = ["Z:\\FY25_LRS_Tx_needs\\runs_for_feb\\example_txneeds_highRE_sensitivities_vsc.csv"]
...
toplot = {
    'Capacity': {'data': dictin_cap, 'colors':aggcolors, 'columns':'tech', 'values':'Capacity (GW)', 'label':'Capacity [GW]'},
...
for slidetitle, data in toplot.items():
    ...
    for col, case in enumerate(cases):
        ...
        df = data['data'][case]
        df = df[df['year']==2040]
        dfplot = df.pivot(index='year', columns=data['columns'], values=data['values'])
    ...
    for col, case in enumerate(cases):
        ...
        dfplot['case'] = case
        df_diff = pd.concat([df_diff,dfplot])
    ...
    net_values = [df_diff.iloc[0:1,0:-1].sum().sum(),df_diff.iloc[1:2,0:-1].sum().sum(),df_diff.iloc[2:3,0:-1].sum().sum()]
    net_values = [df_diff.iloc[0:1,0:-1].sum().sum()]
    f,ax = plt.subplots(
        2, 1, figsize=(4, 6),
        sharex=True, sharey=sharey, dpi=None,
    )
    dfbase[slidetitle].plot(ax = ax[0],kind = 'bar',  stacked = True, color=aggcolors,width = 0.2)
    #dfother[slidetitle].plot(ax = ax[0],kind = 'bar',  stacked = True, color=aggcolors,width = 0.2)
    df_diff.plot(ax = ax[1],x = 'case', kind = 'bar',  stacked = True, color=aggcolors, legend = False,width = 0.2 )
    ax[1].plot(df_diff['case'],net_values,marker= 'o',color = 'white',linestyle = '')
    plt.xlabel(None)
    plt.ylabel('')
    ax[0].legend(loc='center right', bbox_to_anchor=(1.5, 0.5),fontsize = 'small')
    ax[0].set_ylabel(data['label'], y=-0.075)   
    ax[1].set_title(f'Diff from {basecase}')
    ax[0].set_title(f'2040 {basecase}')
...
### Legend
leg = ax[1].legend(
    loc='upper left', bbox_to_anchor=(-0.3,-0.05), frameon=False, fontsize='small',
    handletextpad=0.3, handlelength=0.7,
)
for legobj in leg.legend_handles:
    legobj.set_linewidth(8)
...
for _col, level in enumerate(['transgrp']):
    col = _col + 2
    df = {}
    for case in cases:
