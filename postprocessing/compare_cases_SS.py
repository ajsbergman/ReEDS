...
_caselist = [os.path.join(reeds_path,'postprocessing','example_txneeds-test.csv')]
...
## define a new grouping of cost categories
costcat_groups = {
    'Gen & Stor Capex':'Gen & Stor CapEx',
    'Gen & Stor O&M':'Gen & Stor O&M',
    'Transmission Capex':'Tx. & Ix.',
    'Transmission O&M':'Tx. & Ix.',
    'Interconnection Capex':'Tx. & Ix.',
    'Interconnection O&M':'Tx. & Ix.',
    'PTC': 'Tax Credits',
    'H2 PTC': 'Tax Credits',
    'ITC':'Tax Credits',
    'ACP':'Other',
...
for case in tqdm(cases, desc='NPV of system cost'):
    ...
    dictin_npv[case].index = pd.Series(dictin_npv[case].index).replace(costcat_groups)
...
for case in tqdm(cases, desc="NEUE jitter data"):
    target_year = 2040
    infiles = sorted(glob(os.path.join(cases[case],'outputs', f'neue_{target_year}i*.csv')))
    if not len(infiles):
        continue
    last_iter_file = infiles[-1]
    df = pd.read_csv(last_iter_file)

    ## national-level neue
    dictin_neue_jitter['national'][case] = df.loc[
        (df.level =='country') & (df.metric =='sum'),
        ['NEUE_ppm']
    ].iloc[0]['NEUE_ppm']

    ## regional-level neue
    dictin_neue_jitter['regional'][case] = df.loc[
        (df.level =='transgrp') & (df.metric =='sum'),
        ['region', 'NEUE_ppm']
    ].set_index('region')['NEUE_ppm']
...
plt.close()
f,ax = plt.subplots(
    2, 3, figsize=(width, width*1.15), sharex=True,
    sharey=False,)
handles = {}
...
for col in range(3):
    ax[1,col].set_xticks(range(len(cases)))
    ax[1,col].set_xticklabels(cases.keys(), rotation=45)
    ax[1,col].annotate('Diff', (0.03,0.03), xycoords='axes fraction', fontsize='large')
    ax[1,col].axhline(0, c='k', ls='--', lw=0.75)
...
for x, case in enumerate(dfcost_npv.index):
    ...
    if len(cases) <= 10:
        ax[col].annotate(
            ...
            (x, val - _ypad), ha='center', va='top', color='k', size=14,
            path_effects=[pe.withStroke(linewidth=2.0, foreground='w', alpha=0.8)],
        )
    else:
        ax[col].annotate(
            f'{np.around(val,-1):.0f}',
            (x, val + _ypad), ha='center', va='bottom', color='k', size=14,
            rotation=45,
        )
...
# determine plot dimensions based on number of cases
num_cases = len([case for case in cases if (case in dictin_neue_jitter['regional'] or case in dictin_neue_jitter['national'])])
figwidth = min(13.33, max(6, num_cases * 1.2))
figheight = 6.88
fig = plt.figure(figsize=(figwidth, figheight))

### extra space at the top for annotations
ax = fig.add_subplot(111)
fig.subplots_adjust(top=0.85, right=0.85)

target_year = 2040
case_indices = {case: i for i, case in enumerate(cases.keys())}
jitter_width = 0.2

## find max neue value to set y-axis limit
max_neue_value = 0

for case in cases:
    if case not in dictin_neue_jitter['regional']:
        continue

    regional_values = dictin_neue_jitter['regional'][case]
    x_positions = [case_indices[case] + np.random.uniform(-jitter_width, jitter_width) for _ in range(len(regional_values))]
    plt.scatter(
        x_positions, regional_values.values,
        s=40, alpha=0.5,
        color='blue', marker='o', edgecolor='none',
        label='regional-level' if case == list(cases.keys())[0] else "" 
    )

for case in cases:
    if case not in dictin_neue_jitter['national']:
        continue
    
    national_value = dictin_neue_jitter['national'][case]
    max_neue_value = max(max_neue_value, national_value)

    national_value = dictin_neue_jitter['national'][case]
    plt.scatter(
        [case_indices[case]], [national_value],
        s=100, alpha=1,
        color='red', marker='^', edgecolor='none',
        label='national-level' if case == list(cases.keys())[0] else "" 
    )
...
## add annotations for national-level neue values at the top of the chart
y_limit = max(10, max_neue_value * 1.1)
...
plt.sca(ax)
for case in cases:
    if case in dictin_neue_jitter['national']:
        national_value = dictin_neue_jitter['national'][case]
        ax.annotate(
            f"{national_value:.2f}ppm",
            xy=(case_indices[case], 1), xytext=(0, 20), textcoords = 'offset points', xycoords=('data', 'axes fraction'),
            ha='center', va='center', fontsize = 12, fontweight = 'bold',
            bbox=dict(boxstyle="round,pad=0.3", fc="white", ec="red", alpha=0.8)
        )

## formatting
plt.ylabel('NEUE (ppm)', fontsize=14)
plt.xticks(range(len(cases)), list(cases.keys()), fontsize=12, rotation=45, ha='right')

handles, labels = plt.gca().get_legend_handles_labels()
by_label = dict(zip(labels, handles))
plt.legend(by_label.values(), by_label.keys(), loc='upper left', bbox_to_anchor=(1.02, 1), fancybox=True, shadow=False)

plt.tight_layout()

## add to a slide
slide = add_to_pptx('2040 NEUE - regional and national')
if interactive:
    plt.show()
...
try:
    f, ax, dfplot = reedsplots.plot_max_imports(
        ...
        colors={v: colors[k] for k,v in cases.items()},
        casenames={v:k for k,v in cases.items()},
        level='transgrp', tstart=startyear,draw_limit=True
    )
    ### Save it
    slide = add_to_pptx(
    ...
    for subtract_baseyear in [None, 2020]:
        ...
        for case in cases:
            ...
            reedsplots.plot_trans_onecase(
                ...
                thickborders='transgrp', drawstates=False, drawzones=False, 
    ...
    for case in cases:
        ...
        if case == basecase:
            ...
            reedsplots.plot_trans_onecase(
                ...
                thickborders='transgrp', drawstates=False, drawzones=False, 
                label_line_capacity=1,
                scale=(True if case == basecase else False),level = 'transgrp',
            )
        else:
            ### Plot the difference
            reedsplots.plot_trans_diff(
                ...
                thickborders='transgrp', drawstates=False, drawzones=False, 
                label_line_capacity=1,
                scale=False,level = 'transgrp', justdiff=True
            )
    ### Formatting
...
#%% Regional interfaces plot
plt.close()
interface_level = 'transgrp'
base_year = 2020

## create a dataframe to store transmission capacity by interface
interface_builds = {}

for case in cases:
    df_current = dictin_trans_r[case].loc[dictin_trans_r[case].t == lastyear].copy()
    df_base = dictin_trans_r[case].loc[dictin_trans_r[case].t == base_year].copy()

    ### map r,rr to higher level regions
    df_current['r_region'] = df_current.r.map(hierarchy[case][interface_level])
    df_current['rr_region'] = df_current.rr.map(hierarchy[case][interface_level])
    df_base['r_region'] = df_base.r.map(hierarchy[case][interface_level])
    df_base['rr_region'] = df_base.rr.map(hierarchy[case][interface_level])
    ### keep only interregional lines
    df_current = df_current.loc[df_current.r_region != df_current.rr_region].copy()
    df_base = df_base.loc[df_base.r_region != df_base.rr_region].copy()

    ### create dictionary with names of all interfaces
    def make_interface_name(row):
        return f"{row.r_region} - {row.rr_region}"
    
    df_current['interface'] = df_current.apply(make_interface_name, axis=1)
    df_base['interface'] = df_base.apply(make_interface_name, axis=1)
    ### sum transmission capacity by interface for beginning and end year
    current_cap = df_current.groupby('interface').MW.sum() / 1000
    base_cap = df_base.groupby('interface').MW.sum() / 1000
    new_builds = current_cap.subtract(base_cap, fill_value=0)
    interface_builds[case] = new_builds  ## store results

df_interface_builds = pd.concat(interface_builds, axis=1).fillna(0)

### only keep interfaces with builds > 2.5GW
min_build = 2.5
df_interface_builds = df_interface_builds.loc[df_interface_builds.max(axis=1) >= min_build]
### sort by total builds
df_interface_builds = df_interface_builds.loc[df_interface_builds.max(axis=1).sort_values(ascending=True).index]

## Plot
fig, ax = plt.subplots(figsize=(4.5,6))
y = np.arange(len(df_interface_builds))
x_max = df_interface_builds.values.max() * 2
markers = ['o', 's', 'd', '^', 'v', '<', '>', 'p', '*', 'h']
marker_size = 15
...
for i, case in enumerate(cases):
    ...
    scatter = ax.scatter(df_interface_builds[case].values, y, s=marker_size, marker=marker, color=colors[case], edgecolors='black', linewidths=marker_edge_width, alpha=0.8, label=case, zorder=10)

## add labels
ax.set_xlabel('Capacity [GW]', fontsize=10, weight='bold')
ax.set_yticks(y)
ax.set_yticklabels(df_interface_builds.index, fontsize=8)
...
if len(cases) <= 5:
    for i, case in enumerate(cases):
        for j, val in enumerate(df_interface_builds[case].values):
            if val >= 10:
                offset_y = 0.2 * (i - len(cases)/2 + 0.5) / len(cases)
                ax.text(val + x_max * 0.06, y[j] + offset_y, f'{val:.1f}', ha='left', va='center', fontsize=7, color=colors[case], fontweight='bold', path_effects=[pe.withStroke(linewidth=3, foreground='white')])

plt.tight_layout()

slide = add_to_pptx(f'New Transmission Builds by {interface_level.title()} Interface')
if interactive:
    plt.show()
...
for i, case in enumerate(cases):
    ...
    legend_ax.scatter(
        [-10], [-10],
        s=marker_size, marker=marker, color=colors[case], edgecolors='black', linewidths=marker_edge_width, label=case
    )
## create a standalone legend
legend = legend_ax.legend(
    loc='center', frameon=True, framealpha=0.9, facecolor='white', edgecolor='lightgray', fontsize=10
)
legend.get_title().set_fontweight('bold')
## add legend to the same slide
add_to_pptx(slide=slide, width=2, height=1.5, left=11, top=5.5)
if interactive:
    plt.show()
