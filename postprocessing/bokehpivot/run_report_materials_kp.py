...
data_source = f'{bokehpivot_dir}/reeds_scenarios_materials.csv' #either a scenarios.csv file or ReEDS run directory (or directories separated by pipe symbols).
base = pd.read_csv(data_source)['name'][0] #Name of base case for when diff='Yes'. Defaults to first case in reeds_scenarios.csv.
output_dir = f'{bokehpivot_dir}/out/reeds_report_materials' #This is the directory that will be created to contain the report. If it already exists, the existing directory will be archived with a date.
data_type = 'ReEDS 2.0'
