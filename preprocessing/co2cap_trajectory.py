...
for region in sorted(hierarchy[level].unique()):
    ### Get BAs to sum
    r = hierarchy.loc[hierarchy[level]==region].index.values
    ### Get past emissions
...
if not apikey:
    raise ValueError(
        ...
        "1. Register for a free API key at https://www.eia.gov/opendata/register.cfm\n"
        "2. Set the EIA_API_KEY environment variable with your key"
    )
