...
def get_inputs(sw):
    ...
    tech_map.raw = reeds.reedsplots.simplify_techs(tech_map.raw, display_level = 'diagnostics')
    ...
    try:
        ...
        for key in ['gencap', 'storcap', 'genstorcap', 'genfailrate', 'genrepairrate']:
            ...
            if 'i' in pras_system[key].columns.names:
                col_vals = pras_system[key].columns.get_level_values('i').unique().tolist()
                mapping = dict(zip(col_vals, reeds.reedsplots.simplify_techs(col_vals)), display_level = 'diagnostics')
                pras_system[key].rename(columns = mapping, level = 'i', inplace = True)
...
def plot_pras_load_units(sw, dfs):
    ...
    units = cap.loc[
            ~cap.i.isin(
                reeds.reedsplots.simplify_techs(vre_gen.columns.get_level_values('i').unique(), display_level = 'diagnostics'))
            & (cap.MW > 0)
        ]
    ## Get the colors
    tech_style = dfs['tech_style']['color'].squeeze()
