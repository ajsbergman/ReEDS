...
def get_model_dimensions(casedir, t):
    """
    Get final year GAMS model statistics from final lst file
    Reports:
    - number of single equations
    - number of single variables
    - non-zero elements in the constraint matrix
    - peak memory usage in GB (as reported in profile summary)
    ...
    """
    model_stats = {'equations':0,
                'variables':0,
                'non_zero_elements':0,
                'peak_memory':0
                }

    file_path = sorted(glob(os.path.join(casedir,'lstfiles',f'*{t}i*.lst')))[-1]
    profile_memory = []
    profile_summary = False
    with open(file_path, 'r') as f:
        for line in f:
            if 'SINGLE EQUATIONS' in line:
                model_stats['equations'] = int(line.split()[6].replace(',',''))
            if 'SINGLE VARIABLES' in line:
                model_stats['variables'] = int(line.split()[6].replace(',',''))
            if 'NON ZERO ELEMENTS' in line:
                model_stats['non_zero_elements'] = int(line.split()[3].replace(',',''))
            if profile_summary:
                profile_memory.append(float(line.split()[1][:-2]))
            if '---- Profile Summary' in line:
                profile_summary = True
        model_stats['peak_memory'] = max(profile_memory)
    return model_stats
